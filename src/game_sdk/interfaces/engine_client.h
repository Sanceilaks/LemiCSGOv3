...
{
public:
	virtual int get_intersecting_surfaces(const model_t* model, const Vector& vCenter, const float radius,
	...
	virtual i_material* trace_line_material_and_lighting(const Vector& start, const Vector& end, Vector& diffuseLightColor,
		Vector& baseColor) = 0;
	...
	virtual void get_viewangles(Vector& va) = 0;
	virtual void set_viewangles(Vector& va) = 0;
	...
	virtual const D3DMATRIX& world_to_screen_matrix() = 0;
	virtual const D3DMATRIX& world_to_view_matrix() = 0;
	...
	virtual void compute_lighting(const Vector& pt, const Vector* pNormal, bool bClamp, Vector& color,
	...
	virtual void compute_leaves_connected(const Vector& vecOrigin, int nCount, const int* pLeafIndices, bool* pIsConnected)
		= 0;
	...
	virtual void sphere_paint_surface(const model_t* model, const Vector& location, unsigned char chr, float fl1,
		float fl2) = 0;
	...
	virtual void sphere_trace_paint_surface(const model_t* model, const Vector& position, const Vector& vec2, float radius,
		/*CUtlVector<unsigned char, CUtlMemory<unsigned char, int>>*/
		int& utilVecShit) = 0;
