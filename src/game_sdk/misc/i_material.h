...
public:
	...
	virtual PreviewImageRetVal_t get_preview_image_properties(int* width, int* height,
		ImageFormat* imageFormat, bool* isTranslucent) const = 0;

	// Get a preview image at the specified width/height and bitDepth.
	// Will do resampling if necessary.(not yet!!! :) )
	// Will do color format conversion. (works now.)
	virtual PreviewImageRetVal_t get_preview_image(unsigned char* data,
	...
	virtual VertexFormat_t	get_vertex_format() const = 0;
	...
	virtual bool			get_property_flag(MaterialPropertyTypes_t type) = 0;
	...
	virtual MorphFormat_t	get_morph_format() const = 0;

	// fast find that stores the index of the found var in the string table in local cache
	virtual IMaterialVar* find_var_fast(char const* pVarName, unsigned int* pToken) = 0;

	// Sets new VMT shader parameters for the material
	virtual void			set_shader_and_params(KeyValues* pKeyValues) = 0;
	...
	virtual IMaterial* check_proxy_replacement(void* proxyData) = 0;
